...
function newGame() {
	...
	for (i = 0; i < wordLength; i++) {
		processingChars(frag, wordToGuess[i])
	}
   
    var i = 0 // Adding 10 other characters

    while (i < wordLength){

     rand_char = abc[parseInt(Math.random()* abc.length)];    	
     rand_char = rand_char.toLowerCase()
     for (a =0; a < wordToGuess.length; a++){
     	if (rand_char == wordToGuess[a]){
     		Console.log("Word exists so I ain't doing shit here!")
     	}else{
     		processingChars(frag, rand_char)
     	 	i++
     	 	rand_char = abc[parseInt(Math.random()* abc.length)];    	
    	    rand_char = rand_char.toLowerCase()
     	}
...
function processingChars(words,frag,char){
   var all_words = arr[words.length * 2]
